...
const submit = () => {
    form.post(route('working-days.store', props.company.id), {
        onSuccess: () => toast('Working day has been added successfully.'),
...
<template>
    <DefaultLayout>
        <Head :title="`Create Working Day - ${company.name}`"/>
        <v-row no-gutters>
            <v-col cols="12">
                <v-card>
                    <CardTitle
                        ...
                        :title="`Create Working Day - ${company.name}`"
                    />
