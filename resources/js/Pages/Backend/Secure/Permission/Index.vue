...
const getData = (obj) => {
    setLimit(obj);
    axios.get(route('core.permission.get', state.filters)).then(({data}) => {
        state.loading = false;
...
const toggleStatus = (item) => {
    axios.get(route('core.permission.toggle-status', item.id), {}, {preserveScroll: true})
    ...
    <DefaultLayout>
        ...
        <v-row no-gutters>
            <v-col cols="12">
                <v-card>
                    <CardTitle
                        :extra-route="{title: 'Back' , route: 'dashboard', icon:'mdi-arrow-left-bold'}"
                        icon="mdi-plus"
                        title="Permissions"
                    />
                    ...
                    <v-card-text>
                        ...
                        >
                            ...
                            <template v-slot:item.actions="{ item }">
                                <btn-link
                                    :route="route('core.permission.edit', item.id)"
