...
const getData = (obj) => {
    setLimit(obj);
    axios.get(route('company.departments.get', state.filters)).then(({data}) => {
        state.loading = false;
...
const toggleStatus = (item) => {
    axios.post(route('company.departments.toggle-status', item.id))
    ...
    <DefaultLayout>
        ...
        <v-row no-gutters>
            <v-col cols="12">
                <v-card>
                    <CardTitle
                        :router="{title: 'Add New' , route: 'company.departments.create'}"
                        icon="mdi-plus"
                        title="Departments"
                    />
                    ...
                    <v-card-text>
                        ...
                        >
                            ...
                            <template v-slot:item.actions="{ item }">
                                <btn-link
                                    :route="route('company.departments.edit', item.id)"
                                    color="bg-darkprimary"
                                    icon="mdi-pencil"/>

                                <btn-delete
                                    :route="route('company.departments.destroy', item.id)"
